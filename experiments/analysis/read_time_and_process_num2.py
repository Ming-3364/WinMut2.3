...
# only add statistics on exit(42) / exit(-1) 
...
def getRun(filename, casename, skip = 0):
	...
	with open(filename, encoding='utf-8', errors='replace') as f:
		...
		if finalFlag != True:
			...
			#print('num', totnum)
		ret = f"killed: {totnum_killed}/{totnum}({totnum_killed / totnum *100:.2f}%)"
		return run, ret
...
try:
	...
	if len(sys.argv) > 2:
		...
		run, ret = getRun(filename, filename)
		print(run)
		print(ret)
