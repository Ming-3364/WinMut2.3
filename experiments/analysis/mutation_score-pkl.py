...
class MACase:
    ...
    def readProcTree(self, proc_tree_path):
        ...
        with open(proc_tree_path, encoding='utf-8', errors='replace') as f:
            for l in tqdm(f, desc="Proc Tree"):
                l = l.strip()
                if l.startswith('--'):
    ...
    def checkProcOutput(self):
        ...
        for mut_id in tqdm(mut_id_need_check, desc="Check Proc Output"):
            for output_file_name in tqdm(file_need_check, desc="File Progress", leave=False):
    ...
    def checkProcEndStatus(self):
        for proc in tqdm(self.proc_tree, desc="Check Proc End Status"):
    ...
    def getAllMutation(self, all_mutation_path):
        ...
        with open(all_mutation_path, encoding='utf-8', errors='replace') as f:
            for l in tqdm(f, desc="All Mutation"):
                if l.startswith('--'):
                    continue
                elif l.startswith('++'):
                    continue
